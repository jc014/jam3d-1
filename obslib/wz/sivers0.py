...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 16:32:58 2019
...
def _get_FUTWZ(xA,xB,qT,hadronA,hadronB,boson,TransversePolarizationA,TransversePolarizationB,PDFA,PDFB,w_hadronA,w_hadronB):
    """
    ...
    if TransversePolarizationA: # hadronA is transversely polarised  
            ...
            if boson == 'W+':
                ...
                K = -2. * qT * MA / wq
                res = K*conf['aux'].Vud**2 * 2. * uA * dbarB * np.exp(-qT**2 / wq) / (np.pi * wq)  
                
                wq = np.abs(w_hadronA[4]) + np.abs(w_hadronB[1]) # dbar u
                K = -2. * qT * MA / wq
                res += K*conf['aux'].Vud**2 * 2. * dbarA * uB * np.exp(-qT**2 / wq) / (np.pi * wq) 
                
                wq = np.abs(w_hadronA[1]) + np.abs(w_hadronB[6]) # u sbar
                K = -2. * qT * MA / wq
                res += K*conf['aux'].Vus**2 * 2. * uA * sbarB * np.exp(-qT**2 / wq) / (np.pi * wq)  
                
                wq = np.abs(w_hadronA[6]) + np.abs(w_hadronB[1]) # sbar u
                K = -2. * qT * MA / wq
                res += K*conf['aux'].Vus**2 * 2. * sbarA * uB * np.exp(-qT**2 / wq) / (np.pi * wq) 
                ...
                K = -2. * qT * MA / wq
                res = K*conf['aux'].Vud**2 * 2. * ubarA * dB * np.exp(-qT**2 / wq) / (np.pi * wq)  
        
                wq = np.abs(w_hadronA[3]) + np.abs(w_hadronB[2]) # d ubar
                K = -2. * qT * MA / wq
                res += K*conf['aux'].Vud**2 * 2. * dA * ubarB * np.exp(-qT**2 / wq) / (np.pi * wq) 

                wq = np.abs(w_hadronA[2]) + np.abs(w_hadronB[5]) # ubar s
                K = -2. * qT * MA / wq
                res += K*conf['aux'].Vus**2 * 2. * ubarA * sB * np.exp(-qT**2 / wq) / (np.pi * wq)  
                ...
                K = -2. * qT * MA / wq
                res = K*(conf['aux'].cvuz**2 + conf['aux'].cauz**2) * uA * ubarB * np.exp(-qT**2 / wq) / (np.pi * wq)  
                ...
                K = -2. * qT * MA / wq
                res += K*(conf['aux'].cvsz**2 + conf['aux'].casz**2) * sA * sbarB * np.exp(-qT**2 / wq) / (np.pi * wq)  
...
def get_FUTWZ(xA,xB,qT,hadronA,hadronB,boson,TransversePolarizationA,TransversePolarizationB):
    """
    ...
    return _get_FUTWZ(xA,xB,qT,hadronA,hadronB,boson,TransversePolarizationA,TransversePolarizationB,PDFA,PDFB,w_hadronA,w_hadronB)

    
    
def get_FUTWZY(y,qT,energy,hadronA,hadronB,boson,TransversePolarizationA,TransversePolarizationB):
    """
    Unpolarized distribution
    We use notations form Phys.Rev. D79 (2009) 034005 http://inspirehep.net/record/796530
    ...
    # build structure function
    return get_FUTWZ(xA,xB,qT,hadronA,hadronB,boson,TransversePolarizationA,TransversePolarizationB)
